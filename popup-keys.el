...
The %s is replaced by the popup name."
  :group 'popup-keys
  :type 'string)
...
(defvar popup-keys:current-popup-name nil
  "The command name of `popup-keys:current-popup'.")
...
PARAMS.  Does generate the associated command function POPUP-NAME."
...
(defun popup-keys:de-generate (popup-name)
  "Unbind the function POPUP-NAME."
  (fmakunbound popup-name))

(defun popup-keys:generate (popup-name)
  "Generate the command `POPUP-NAME'."
  (eval
   `(defun ,popup-name nil
...
"Define a new popup command `POPUP-NAME'.
