...
This package is not maintained for public distribution.  The *magit-key-mode* 
has become a standalone package called *magit-popup*, which is more or less 
equivalent to this package  That said, it works very well for me, so if
*popup-keys* seems useful to you, feel free to use it.
